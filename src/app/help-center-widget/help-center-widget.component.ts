...
import { HeaderComponent, ChatHeaderComponent } from '../shared/components/header'
...
import { HelpCenterConfigService } from '../services/help-center-config.service'
...
export class HelpCenterWidgetComponent implements OnInit, OnDestroy {
  ...
  private baseUrl = 'https://babylai.net/api'
  ...
  async sendMessage(messageText?: string) {
    ...
    if (!textToSend.trim() || !this.isSignalRConnected || this.isChatClosed) return
  ...
  handleReceiveMessage(message: string, senderType: string, needsAgent: boolean) {
    if (needsAgent) {
